...
class SkillManagementServiceConfigsTest {
    ...
    void parsesDefinitionAndLifecycleStoresFromProperties(@TempDir Path tempDir) {
        ...
        properties.setProperty("andalus.skills.store.directory", tempDir.resolve("definitions").toString());
        properties.setProperty("andalus.skills.lifecycle.store.kind", "jdbc");
        properties.setProperty("andalus.skills.lifecycle.store.jdbc.table-name", "skill_lifecycle");
        properties.setProperty("andalus.skills.lifecycle.store.jdbc.initialize-schema", "false");
        properties.setProperty("andalus.skills.events.store.kind", "filesystem");
        properties.setProperty("andalus.skills.events.store.directory", tempDir.resolve("events").toString());
        properties.setProperty("andalus.skills.events.store.max-events", "25");
        properties.setProperty("andalus.skills.artifacts.store.kind", "filesystem");
        properties.setProperty("andalus.skills.artifacts.store.directory", tempDir.resolve("artifacts").toString());
        properties.setProperty("andalus.skills.lifecycle.reconcile.mode", "bootstrap");
    ...
    void parsesDefinitionAndLifecycleStoresFromEnvironment(@TempDir Path tempDir) {
        SkillManagementServiceConfig config = SkillManagementServiceConfigs.fromEnvironment(Map.of(
                "ANDALUS_SKILLS_STORE_KIND", "s3",
                "ANDALUS_SKILLS_STORE_OBJECT_PREFIX", "tenants/acme/skills",
                "ANDALUS_SKILLS_LIFECYCLE_STORE_KIND", "filesystem",
                "ANDALUS_SKILLS_LIFECYCLE_STORE_DIRECTORY", tempDir.resolve("lifecycle").toString(),
                "ANDALUS_SKILLS_EVENTS_STORE_KIND", "memory",
                "ANDALUS_SKILLS_EVENTS_STORE_MAX_EVENTS", "75",
                "ANDALUS_SKILLS_ARTIFACTS_STORE_KIND", "rustfs",
                "ANDALUS_SKILLS_ARTIFACTS_STORE_OBJECT_PREFIX", "tenants/acme/artifacts",
                "ANDALUS_SKILLS_LIFECYCLE_RECONCILE_MODE", "sync"));
    ...
    void parsesProfiledServiceConfigFromProperties(@TempDir Path tempDir) {
        Properties properties = new Properties();
        properties.setProperty("andalus.skills.profile", "local-filesystem");
        properties.setProperty("andalus.skills.profile.base-directory", tempDir.toString());
        properties.setProperty("andalus.skills.profile.max-events", "33");
    ...
    void parsesProfiledServiceConfigFromEnvironment() {
        SkillManagementServiceConfig config = SkillManagementServiceConfigs.fromEnvironment(Map.of(
                "ANDALUS_SKILLS_PROFILE", "rustfs",
                "ANDALUS_SKILLS_PROFILE_OBJECT_PREFIX", "tenant-a/skills",
                "ANDALUS_SKILLS_PROFILE_MAX_EVENTS", "44"));
    ...
    void profileAllowsExplicitStoreKindOverride(@TempDir Path tempDir) {
        SkillManagementServiceConfig config = SkillManagementServiceConfigs.fromMap(Map.of(
                ...
                "andalus.skills.profile.object-prefix", "tenant-a/skills",
                "andalus.skills.profile.max-events", "55",
                "andalus.skills.artifacts.store.kind", "filesystem",
                "andalus.skills.artifacts.store.directory", tempDir.resolve("artifacts").toString()));
    ...
    void validatesServiceConfigWithoutThrowing() {
        SkillStoreConfigValidationResult result = SkillManagementServiceConfigs.validateMap(Map.of(
                "andalus.skills.events.store.kind", "filesystem"));
    ...
    void validatesEnvironmentConfigWithoutThrowing() {
        SkillStoreConfigValidationResult result = SkillManagementServiceConfigs.validateEnvironment(Map.of(
                "ANDALUS_SKILLS_STORE_KIND", "filesystem"));
