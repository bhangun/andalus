...
import tech.kayys.andalus.memory.executor.AbstractMemoryExecutor;
import tech.kayys.andalus.memory.executor.MemoryOperationType;
...
import tech.kayys.andalus.memory.model.Memory;
import tech.kayys.andalus.memory.model.MemoryType;
import tech.kayys.andalus.memory.service.VectorMemoryStore;
import tech.kayys.andalus.memory.spi.EmbeddingService;
...
public class LongTermMemoryExecutor extends AbstractMemoryExecutor {
     ...
     */
    @ConfigProperty(name = "andalus.memory.longterm.importance.threshold", defaultValue = "0.5")
    double defaultImportanceThreshold;

    /**
     * Decay rate for memory importance (per hour)
     */
    @ConfigProperty(name = "andalus.memory.longterm.decay.rate", defaultValue = "0.01")
    double decayRate;

    /**
     * Default search limit
     */
    @ConfigProperty(name = "andalus.memory.longterm.search.limit", defaultValue = "10")
    int defaultSearchLimit;

    /**
     * Minimum similarity threshold for vector search
     */
    @ConfigProperty(name = "andalus.memory.longterm.min.similarity", defaultValue = "0.7")
