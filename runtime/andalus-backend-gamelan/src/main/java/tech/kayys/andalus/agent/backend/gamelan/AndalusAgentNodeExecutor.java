...
import tech.kayys.andalus.core.DefaultAndalusRuntime;
...
/**
 * WorkflowExecutor implementation allowing Gamelan to invoke Andalus agents as workflow steps.
 */
@ApplicationScoped
public class AndalusAgentNodeExecutor implements WorkflowExecutor {

    private static final Logger LOG = Logger.getLogger(AndalusAgentNodeExecutor.class);
    public static final String EXECUTOR_TYPE = "-agent";

    @Inject
    Instance<DefaultAndalusRuntime> runtimeInstances;

    @Inject
    Instance<WorkflowBindingResolver> bindingResolverInstances;

    private DefaultAndalusRuntime Runtime;
    private WorkflowBindingResolver bindingResolver;

    public AndalusAgentNodeExecutor() {
    }

    public AndalusAgentNodeExecutor(DefaultAndalusRuntime Runtime, WorkflowBindingResolver bindingResolver) {
    ...
    public Uni<NodeExecutionResult> execute(NodeExecutionTask task) {
        ...
        LOG.debugf("Executing Andalus agent step %s with agent %s", task.nodeId().value(), agentId);
        ...
        DefaultAndalusRuntime runtime = resolveRuntime();
        if (runtime == null) {
            return Uni.createFrom().item(SimpleNodeExecutionResult.failure(
                    task.runId(),
                    task.nodeId(),
                    task.attempt(),
                    new ErrorInfo("RUNTIME_UNAVAILABLE", "No AndalusRuntime configured", null, Map.of()),
                    task.token()
            ));
        }
        ...
        return Uni.createFrom().completionStage(runtime.executeAsync(agent, agentReq))
                ...
                .onFailure().recoverWithItem(err -> {
                    LOG.errorf(err, "Failed to execute Andalus agent step %s", task.nodeId().value());
    ...
    private DefaultAndalusRuntime resolveRuntime() {
        if (Runtime != null) {
            return Runtime;
        }
