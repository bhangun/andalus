package tech.kayys.andalus.agent.backend.gamelan;
...
import tech.kayys.andalus.agent.spi.WorkflowBackend;
import tech.kayys.andalus.agent.spi.WorkflowTypes.CreateRunRequest;
import tech.kayys.andalus.agent.spi.WorkflowTypes.ExecutionEvent;
import tech.kayys.andalus.agent.spi.WorkflowTypes.RunHistory;
import tech.kayys.andalus.agent.spi.WorkflowTypes.RunStatus;
import tech.kayys.andalus.agent.spi.WorkflowTypes.Signal;
import tech.kayys.andalus.agent.spi.WorkflowTypes.WorkflowCapabilities;
import tech.kayys.andalus.agent.spi.WorkflowTypes.WorkflowRun;
import tech.kayys.andalus.agent.spi.WorkflowTypes.WorkflowRunId;
...
/**
 * Adapts the current Gamelan workflow SDK to the backend-agnostic Andalus
...
public class GamelanBackendAdapter implements WorkflowBackend {
    ...
    @Override
    public Uni<tech.kayys.andalus.agent.spi.WorkflowTypes.RunResponse> startRun(
            WorkflowRunId runId,
    ...
    @Override
    public Uni<tech.kayys.andalus.agent.spi.WorkflowTypes.RunResponse> suspendRun(WorkflowRunId runId) {
        ensureInitialized();

        return gamelanClient.runs()
                .suspend(runId.id())
                .reason("Suspended by Andalus agent")
                .execute()
                .map(this::toRunResponse)
    ...
    @Override
    public Uni<tech.kayys.andalus.agent.spi.WorkflowTypes.RunResponse> resumeRun(
            WorkflowRunId runId,
            Map<String, Object> inputs) {
        ensureInitialized();
    ...
    @Override
    public Uni<tech.kayys.andalus.agent.spi.WorkflowTypes.RunResponse> cancelRun(
            WorkflowRunId runId,
            String reason) {
        ensureInitialized();
        ...
        return gamelanClient.runs()
                ...
                .replaceWith(() -> new tech.kayys.andalus.agent.spi.WorkflowTypes.RunResponse(
                        runId,
                        null,
    ...
    @Override
    public Uni<tech.kayys.andalus.agent.spi.WorkflowTypes.RunResponse> signalRun(
            WorkflowRunId runId,
            Signal signal) {
        ensureInitialized();
    ...
    private tech.kayys.andalus.agent.spi.WorkflowTypes.RunResponse toRunResponse(RunResponse response) {
        return new tech.kayys.andalus.agent.spi.WorkflowTypes.RunResponse(
