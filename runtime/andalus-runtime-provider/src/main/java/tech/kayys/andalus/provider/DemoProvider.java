...
public final class DemoProvider implements Provider {
    ...
    public void streamChat(
        ...
        boolean lastWasToolResult = last.content.stream().anyMatch(b -> b instanceof tech.kayys.andalus.resource.ContentPart.ToolResult);
        if (lastWasToolResult) {
            streamText(onEvent, "Got it -- the tool finished. (This is the offline demo provider; " +
                    "configure a real provider in `~/.andalus/config.json` for actual AI responses.)");
        ...
        String reply = "This is the **offline demo provider** -- no API key needed.\n\n" +
                ...
                "your active profile's provider in `~/.andalus/config.json`, or run with `--provider anthropic`.\n\n" +
