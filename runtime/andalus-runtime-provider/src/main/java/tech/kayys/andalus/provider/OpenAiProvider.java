...
public final class OpenAiProvider implements Provider {
    ...
    private void appendOpenAiMessages(ChatMessage m, JsonValue msgArr) {
        if (m.role == ChatMessage.Role.ASSISTANT) {
            ...
            for (tech.kayys.andalus.resource.ContentPart b : m.content) {
                if (b instanceof tech.kayys.andalus.resource.ContentPart.Text t) {
                    if (text.length() > 0) text.append('\n');
                    text.append(t.text());
                } else if (b instanceof tech.kayys.andalus.resource.ContentPart.ToolUse tu) {
                    JsonValue fn = JsonValue.object();
                    fn.put("name", tu.name());
            ...
            for (tech.kayys.andalus.resource.ContentPart b : m.content) {
                if (b instanceof tech.kayys.andalus.resource.ContentPart.Text t) {
                    if (text.length() > 0) text.append('\n');
                    text.append(t.text());
                } else if (b instanceof tech.kayys.andalus.resource.ContentPart.ToolResult tr) {
                    if (text.length() > 0) {
