...
import tech.kayys.andalus.spi.sandbox.SandboxConfiguration;
import tech.kayys.andalus.spi.sandbox.SandboxExecutionResult;
...
class LocalProcessSandboxTest {
    ...
    void testLocalSandboxProvider() throws Exception {
        ...
        assertTrue(provider.descriptor().supports(tech.kayys.andalus.spi.sandbox.SandboxType.PROCESS));
        ...
        tech.kayys.andalus.spi.sandbox.SandboxRequest req = new tech.kayys.andalus.spi.sandbox.SandboxRequest(
                "req-sb-1",
                tech.kayys.andalus.spi.sandbox.SandboxType.PROCESS,
                java.util.Set.of("process-isolation"),
                tech.kayys.andalus.spi.sandbox.SandboxLimits.unlimited(),
                tech.kayys.andalus.spi.sandbox.SandboxFilesystem.empty(),
                tech.kayys.andalus.spi.sandbox.SandboxNetwork.disabled(),
                java.util.Map.of("MY_VAR", "my_val"),
                java.util.Map.of()
        );
        try (var s2 = provider.create(req)) {
            assertNotNull(s2);
            assertEquals(tech.kayys.andalus.spi.sandbox.SandboxState.CREATED, s2.state());
            s2.start();
            assertEquals(tech.kayys.andalus.spi.sandbox.SandboxState.RUNNING, s2.state());
    ...
    void testDescriptorAndLifecycle() throws Exception {
        ...
        assertEquals(tech.kayys.andalus.spi.sandbox.SandboxType.PROCESS, sandbox.descriptor().type());
        assertEquals(tech.kayys.andalus.spi.sandbox.SandboxState.RUNNING, sandbox.state());
        assertNotNull(sandbox.context());
        assertTrue(sandbox.context().workspace().isPresent());

        sandbox.destroy();
        assertEquals(tech.kayys.andalus.spi.sandbox.SandboxState.DESTROYED, sandbox.state());
    }
}
