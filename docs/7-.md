...
> **Scope:** How Andalus agents talk to other agents — locally, remotely, and across federated runtime boundaries. Covers the A2A protocol adapter, ANP identity layer, the Agent Network abstraction, durable task execution, and the distributed coordination primitives built on top of them.

## 27.1 Why A2A Exists in Andalus

Andalus is not just a single-agent runtime. A Andalus runtime can **expose** an agent to the outside world, **consume** another agent, and **coordinate** many agents across machines. Three modules make this possible:

| Module | Role | Protocol |
|---|---|---|
| `andalus-a2a` | A2A 1.0 server/client adapter, durable task ledger, workflow bridge | A2A (JSON-RPC / SSE) |
| `andalus-anp` | Agent Network Protocol (DID:WBA identity, signed requests, meta-protocol negotiation) | ANP 1.1 |
| `andalus-agent-network-api` / `-core` | Protocol-neutral abstraction for discovering, routing, and invoking remote agents | Any |
...
```
┌────────────────────────────────────────────────────────────┐
│  Remote A2A Peer (another framework, another Andalus node)  │
└────────────────────────────────────────────────────────────┘
...
├────────────────────────────────────────────────────────────┤
│  A2AMessageMapper — protocol ⇄ Andalus semantic mapping     │
├────────────────────────────────────────────────────────────┤
│  DurableA2ATaskLedger + RemoteAgentWaitState + Journal     │
├────────────────────────────────────────────────────────────┤
│  AndalusRuntime.executeAsync(...)                            │
└────────────────────────────────────────────────────────────┘
```
...
- `A2AServer` (`andalus-a2a/api/A2AServer.java`) — `getAgentCard`, `sendMessage`, `getTask`, `cancelTask`.
- `A2AClient` (`andalus-a2a/api/A2AClient.java`) — the mirror image for outbound calls.
...
`A2AAgentAdapter` (`andalus-a2a/adapter/A2AAgentAdapter.java`) is the simplest adapter:

- Wraps a `AndalusRuntime` + `AgentDefinition` + optional `ExecutionJournal`.
...
`DurableA2AAgentAdapter` (`andalus-a2a/durable/`) replaces the in-memory map with a `DurableA2ATaskLedger`:
...
sequenceDiagram
    ...
    participant Runtime as AndalusRuntime
...
`DurableA2ATaskLedger` (`andalus-a2a/durable/DurableA2ATaskLedger.java`) is the heart of durable A2A:
...
`RemoteAgentWaitState` (`andalus-a2a/durable/RemoteAgentWaitState.java`) is the "park the thread" primitive:
...
## 27.7 Mapping Between A2A and Andalus

`A2AMessageMapper` (`andalus-a2a/adapter/A2AMessageMapper.java`) is the **only** place where A2A model ↔ Andalus model conversion happens:
...
Similarly, `A2AAgentCardMapper` (`andalus-a2a/descriptor/A2AAgentCardMapper.java`) projects the canonical `AgentDescriptor` into an A2A 1.0 `AgentCard` — but only in that direction.
...
`A2AClientTool` (`andalus-a2a/tool/A2AClientTool.java`) exposes an outbound A2A call as a **Andalus `Tool`**. That means an agent can decide to call another agent just like it decides to call any other tool:
...
`DistributedEventRelay` (`andalus-a2a/distributed/DistributedEventRelay.java`) is the local switchboard for cross-agent observability:
...
`A2AWorkflowBridge` (`andalus-a2a/workflow/A2AWorkflowBridge.java`) connects **workflow lifecycle** to **A2A task semantics** without compiling against `andalus-workflow`:
...
ANP (`andalus-anp`) is Andalus's **identity + federation** layer. Where A2A is about the task protocol, ANP is about:
...
`DidWbaIdentity` (`andalus-anp/identity/DidWbaIdentity.java`) parses the format:
...
`AnpMetaProtocolNegotiator` picks the best common protocol between two peers, prioritizing `a2a/1.0` for cross-framework interop and falling back to `anp/1.1` native format. `AnpCapabilityAdvertisement.defaultAndalus(...)` advertises `["a2a/1.0", "anp/1.1"]` by default.

```mermaid
sequenceDiagram
    autonumber
    participant A as Agent A (Andalus)
...
`AnpSecurityMapper` converts `DidWbaIdentity` + `AnpHttpSignature` + optional delegation into a `SecurityContextSnapshot` — the protocol-neutral security type used everywhere else in Andalus. So ANP inbound requests flow into the same authorization path as local ones.

## 27.12 The Agent Network Abstraction

`andalus-agent-network-api` provides a **protocol-neutral** layer above A2A/ANP:
...
`DefaultAgentNetwork` (`andalus-agent-network-core/DefaultAgentNetwork.java`) implements the routing pipeline:
...
The **Local** protocol (`andalus-protocol-local`) is the in-process fast path:
...
The `andalus-knowledge-runtime` module builds a **secure, capability-negotiated, framed** protocol for exchanging evidence artifacts and verified answers between runtimes. This is the most advanced distributed layer in Andalus and is worth a brief tour:
...
## 27.14 Tutorial: Expose a Andalus Agent over A2A

**Goal:** Serve a local Andalus agent at `https://myhost/.well-known/agent-card.json` and accept A2A task requests.
...
## 27.15 Tutorial: Call a Remote Agent from Andalus
...
Two Andalus runtimes each hold different knowledge bases. Both expose `DurableA2AAgentAdapter`. A coordinating agent calls both via `A2AClientTool`, then fuses answers. `DistributedEventRelay` streams status to a monitoring dashboard.
...
## 27.18 Failure Modes & How Andalus Handles Them
...
- A2A in Andalus has **two adapters**: volatile (`A2AAgentAdapter`) and durable (`DurableA2AAgentAdapter`).
- Durability comes from `DurableA2ATaskLedger` + `RemoteAgentWaitState` + resume anchors.
- The **A2A↔Andalus mapping is one-way** and lives only in `A2AMessageMapper` / `A2AAgentCardMapper`.
...
- The knowledge-exchange layer (`andalus-knowledge-runtime`) generalises this to **capability-negotiated, framed, resumable, mutually-trusted** artifact exchange — the future of cross-runtime federation.
...
> **Scope:** How Andalus tells you what it's doing — structured logs, metrics, traces, health checks, audit, and the `ExecutionEvent`/`ExecutionMetrics` event ledger inside the kernel.

## 28.1 The Three Pillars, the Andalus Way
...
`TracingService` (`andalus-observability/tracing/TracingService.java`) is the SPI. `OpenTelemetryTracingService` (`andalus-observability/tracing/OpenTelemetryTracingService.java`) is the default implementation, built on the **global** `GlobalOpenTelemetry` so the SDK is configured externally (e.g. by the Quarkus OTel extension).
...
The core metrics SPI is `Metrics` (`andalus-observability/telemetry/Metrics.java`):
...
`LogEvent` (`andalus-observability/telemetry/LogEvent.java`) is the structured log record:
...
For compliance-grade auditing, `AuditLogger` (`andalus-observability/telemetry/audit/AuditLogger.java`) has a single method:
...
`Slf4jAuditLogger` emits JSON to the `andalus.audit` logger and **masks sensitive keys** (`password`, `secret`, `token`, `key`, `authorization`, `credential`) by default. `setMaskingEnabled(boolean)` toggles this.

For persistence, `ProvenanceStore.store(AuditPayload)` writes audit events to MongoDB/PostgreSQL/etc. `DefaultProvenanceStore` is a no-op `@DefaultBean` so Andalus runs without a backing store.

## 28.5 The Execution Event Ledger

Inside the runtime kernel, `andalus-runtime-spi/execution/event/` defines the **durable append-only event ledger**:
...
`HealthCheck` (`andalus-observability/health/HealthCheck.java`) → `HealthResult` (`HEALTHY | DEGRADED | UNHEALTHY | UNKNOWN`).
...
- All of it is **SPI-first**: swap in Prometheus, Grafana, Splunk, your own backend — Andalus doesn't care.

---

# Chapter 29 — Security, Identity & Multi-Tenancy

> **Scope:** The full Andalus security model — authentication, authorization, delegation, obligations, policy enforcement, tenant isolation, and how it interacts with A2A/ANP.
...
`AuthorizationRequest` is expressed in **Andalus semantics** — `(securityContext, capability, action, resource, attributes)` — never in HTTP verbs or SQL. `AuthorizationRequest.execute(sc, capability)` is the common case.
...
The `SecurityContext` is extracted from `ProtocolContext.attributes().get("andalus.security.context")`; if absent, `SecurityContext.anonymous()`.
...
There is also a parallel `andalus-security-core/obligation/ratelimit/RateLimiter` used by the security obligation pipeline (per-key permits).
...
`ExecutionBudget` (`andalus-runtime-spi/execution/ExecutionBudget.java`) is the master limit record:
...
# Chapter 31 — Tutorial: Build a Complete Andalus Application
...
public final class RepoCopilotWorkflow {
    public static void main(String[] args) throws Exception {
        var runtime = DefaultAndalusRuntime.create();
...
- A Andalus app is: **tools + agent + workflow + security + observability**.
...
> **Scope:** Realistic deployment patterns, each with a Mermaid diagram and a summary of which Andalus modules are involved.
...
flowchart LR
    ...
    IDE -->|LocalRuntime| RT[AndalusRuntime]
    RT --> Agent[ReActAgent]
...
**Modules:** `andalus-agent`, `andalus-core`, `andalus-tool`, `andalus-context`, `andalus-provider`.

**Why Andalus:** Single-JVM, no server, tools run in-process, event ledger gives local audit.
...
**Modules:** `andalus-a2a`, `andalus-security-core`, `andalus-observability`, `andalus-knowledge-runtime`, `andalus-guardrails-runtime`.
...
**Modules:** `andalus-anp`, `andalus-knowledge-runtime`, `andalus-a2a` (for task protocol), `andalus-security-core`.
...
**Modules:** `andalus-hitl-runtime`, `andalus-a2a` (for durable tasks), `andalus-audit`, `andalus-governance`.

## 32.5 Use Case 5 — CI/CD Headless Runner

```bash
java -jar andalus-runner.jar --model gemini-flash --behavior FAST "Summarise README"
```

```mermaid
sequenceDiagram
    participant CI as CI/CD
    participant R as AndalusAgentRunner
    participant S as AgentExecutionService
    participant E as AgentExecution
    participant A as Agent

    CI->>R: andalus-runner "prompt"
...
**Modules:** `andalus-agent-runner`, `andalus-runtime-spi`, `andalus-core`.
...
**Modules:** `andalus-rag`, `andalus-knowledge-runtime`, `andalus-guardrails-runtime`, `andalus-embedding`, `andalus-vector`.
...
**Modules:** `andalus-a2a`, `andalus-anp`, `andalus-agent-network-core`, `andalus-registry`.
...
**Modules:** `andalus-agent/orchestration` (`AgentOrchestrator`), `andalus-agent/orchestration/graph` (`WorkflowGraph`, `CompiledWorkflow`).

## 32.9 Chapter Summary

- Andalus supports a spectrum: **single-JVM → hosted platform → federated knowledge → regulated workflows**.
...
| Module | Purpose |
|---|---|
| `andalus-core` | `AgentDefinition`, `AgentContext`, `AndalusRuntime`, `AgentRequest/Response` |
| `andalus-agent` | `Agent`, `AndalusAgentListener`, ReAct / Plan&Solve / Reflection / Research agents |
| `andalus-agent-runner` | Headless CLI runner (`AndalusAgentRunner`) |
| `andalus-a2a` | A2A 1.0 adapter, durable tasks, workflow bridge, distributed relay |
| `andalus-anp` | DID:WBA identity, signed requests, meta-protocol negotiation |
| `andalus-agent-network-api/core` | Protocol-neutral agent network abstraction |
| `andalus-protocol-local` | In-process protocol fast path |
| `andalus-communication-api/core` | `AgentCommunicator`, `ProtocolRouter`, `AgentProtocol` |
| `andalus-execution-api/core` | Pipeline, interceptors, cancellation, lifecycle |
| `andalus-runtime-spi` | `AgentExecution`, `CheckpointStore`, `ExecutionBudget`, `EventLedger`, `ExecutionCache` |
| `andalus-context` | Context compiler, skeletonizer, symbol resolver, budgeted compiler |
| `andalus-knowledge` | Knowledge items, evidence, resolver, mutation, snapshot, seal, replay |
| `andalus-knowledge-runtime` | Concrete runtime for knowledge (exchange, federation, fusion, resolution) |
| `andalus-memory` | Memory providers, working/episodic/semantic/procedural tiers |
| `andalus-rag` | RAG pipeline, chunker, embedder, retrieval strategies |
| `andalus-vector` | Vector store SPI |
| `andalus-embedding` | Embedding service, providers (hash, tfidf, chargram, gollek) |
| `andalus-graph` | Graph store SPI (in-memory, Neo4j) |
| `andalus-guardrails` | Detectors, policies, plugin registry |
| `andalus-guardrails-runtime` | CDI runtime for guardrails, tool guardrail check |
| `andalus-hitl` | Human task domain, DTOs, strategies |
| `andalus-hitl-runtime` | HITL runtime: repository, REST resource, notification, escalation |
| `andalus-security-api` | Authn, authz, delegation, obligations, propagation, transform |
| `andalus-security-core` | Concrete security: policy engine, PEP, obligation pipeline, decorators |
| `andalus-observability` | Tracing, metrics, logs, audit, health |
| `andalus-observability-otel` | OpenTelemetry integration, `OtelAgentListener` |
| `andalus-resilience` | Circuit breaker, retry |
| `andalus-rate` | Token-bucket rate limiter |
| `andalus-audit` | Audit events, providers, queries, stats |
| `andalus-governance` | Evaluation, guardrails (legacy), policy preflight |
| `andalus-cache` | Project-scoped cache store |
| `andalus-project` | Project/session persistence |
| `andalus-configuration` | Configuration resources, sources, watcher |
| `andalus-event` | Event types, sources, payloads |
| `andalus-messaging` | Message queue service |
| `andalus-workflow` | Workflow definition, engine, steps, transitions |
| `andalus-tool` | Tool SPI (`Tool`, `ToolDescriptor`, `ToolResult`, `ToolRegistry`) |
| `andalus-skill` | Skill SPI |
| `andalus-spi` | Base SPI: `Agent`, `Skill`, `Tool`, plugin, sandbox, memory, inference/workflow backend |
| `andalus-plugin` | Plugin SPI, executor SPI, node SPI |
| `andalus-extension` | Resource, Extension, Metadata, Principal, Version, Reference |
| `andalus-common` | Error codes, validation, migration |
| `andalus-error-spi` | Error codes (enum), exception, JAX-RS mappers |
| `andalus-mcp` | Model Context Protocol client (HTTP + STDIO) |
| `andalus-registry` | Standard registry (A2A/A2UI/Agentic Commerce) |
| `andalus-research` | Research sessions, steps, artifacts |
| `andalus-shaker` | Tree-shaking / packaging for standalone agent JARs |
| `andalus-orchestration` | Multi-agent orchestration + graph engine |
| `andalus-command` | Command discovery API |
| `andalus-graph-runtime` | Graph query/upsert workflow nodes |
...
| **Kernel** | The Andalus Execution Kernel — event ledger, cache, checkpoint, artifacts |
...
The full Andalus codebase is organised into ~30 runtime modules. To go deeper:

1. **Start with** `andalus-core` (`AgentDefinition`, `AndalusRuntime`) and `andalus-agent` (`BaseReActAgent`).
2. **Then read** `andalus-runtime-spi` (`EventLedger`, `ExecutionBudget`, `ExecutionCache`) to understand the kernel.
3. **Then `andalus-a2a`** for distributed execution, and `andalus-security-core` for the security model.
4. **Then `andalus-knowledge-runtime`** for the most advanced federation layer.
...
- **Fill in any specific earlier chapter** you feel was thin (e.g. a deeper dive on `andalus-context` or `andalus-knowledge`)?
