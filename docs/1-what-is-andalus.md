...
> **Andalus** is an extensible agent runtime and framework for building, executing, coordinating, and integrating intelligent agents through a modular execution architecture.

Andalus provides the runtime foundation between an **agent definition** and the systems required to execute that agent: models, tools, memory, context, orchestration, workflows, checkpoints, events, and external agents.
...
The runtime implementation currently exposes `DefaultAndalusRuntime` as the top-level Andalus execution entry point. Its synchronous execution path creates an execution context, delegates to the execution engine, and maps the resulting execution result into an `AgentResponse`; asynchronous execution runs the same path asynchronously.

---

## 1.1 The Problem Andalus Solves
...
Andalus separates these concerns into runtime components and extension points.
...
This separation is one of the foundations of Andalus.

---

# 1.2 Andalus in One Sentence

A concise definition is:

> **Andalus is a modular runtime kernel that turns agent definitions and requests into controlled, observable, extensible executions.**
...
# 1.3 Andalus Is a Runtime, Not Just an LLM Wrapper

Andalus should not be understood as simply:
...
This means Andalus is intended to provide the **execution substrate** on which different agents and higher-level products can be built.

---

# 1.4 The Core Andalus Abstraction
...
# 1.5 The Andalus Execution Stack
...
This execution chain is explicitly described by the current `DefaultAndalusRuntime` implementation.

The important architectural observation is that **the model is only one component of the execution loop**.

---

# 1.6 Andalus's Agent-Centric Model

Andalus treats the agent as a runtime entity rather than merely a prompt.
...
Andalus distinguishes between an agent's **capabilities** and the mechanisms used to execute those capabilities.
...
Andalus's agent runtime includes a memory abstraction rather than requiring every agent to implement memory independently.
...
Andalus therefore provides context-related abstractions including:
...
Andalus is designed around provider and model-routing abstractions rather than embedding a single model implementation into the agent kernel.
...
Andalus's architecture is suitable for deployments where inference may be performed locally or through an external provider.
...
Andalus does not conceptually treat an execution as a single function call.
...
This is what allows later Andalus subsystems to reason about:
...
sequenceDiagram
    ...
    participant Caller
    participant Runtime as Andalus Runtime
...
# 1.14 Andalus and Workflows

Andalus is not limited to one conversational agent turn.
...
The Gamelan adapter maps Andalus's backend-neutral `WorkflowBackend` contract onto the Gamelan SDK and exposes workflow operations such as:
...
Andalus can therefore provide an agent runtime while delegating durable workflow execution to an external or replaceable workflow backend.

---

# 1.15 Andalus and Distributed Agents

Andalus also contains A2A-oriented infrastructure for exposing and communicating with agents.
...
For example, the A2A adapter maps incoming A2A messages into Andalus `AgentRequest` objects and invokes `AndalusRuntime.executeAsync(...)`.
...
Thus Andalus can operate both as:
...
# 1.16 Andalus Is Designed Around Abstraction Boundaries

One of the most important architectural characteristics of Andalus is that infrastructure capabilities are represented by abstractions.
...
# 1.17 Andalus's Layered Architecture
...
flowchart TB
    ...
    B["Andalus Agent Model"]
    C["Andalus Execution Kernel"]
...
The important boundary is between the **Andalus kernel** and the systems surrounding it.

---

# 1.18 What Belongs in Andalus?

Andalus should provide generic runtime infrastructure.
...
Domain-specific business logic should remain outside the generic Andalus kernel.
...
This keeps the core reusable and prevents Andalus from becoming a framework containing every possible business domain.

---

# 1.20 Andalus as a Platform Foundation
...
# 1.21 Andalus's Execution Philosophy
...
Andalus provides the reusable execution substrate; domain applications provide domain-specific behavior.

---

# 1.22 Minimal Mental Model

A developer can initially understand Andalus using this model:
...
This documentation describes Andalus as a complete runtime architecture, including:
...
This chapter establishes the terminology used throughout the remaining Andalus documentation.
...
| **Integrated**  | Andalus abstraction is connected to another runtime/backend    |
...
This distinction is important because the documentation should describe the actual Andalus architecture without presenting future capabilities as already complete.

---

# 1.26 Summary

Andalus is an **agent execution framework and runtime kernel** designed to provide a reusable foundation for intelligent agents.
...
Around that execution loop, Andalus provides or abstracts:
...
The next chapter will define the architectural principles that govern Andalus: modularity, abstraction boundaries, provider neutrality, local/cloud inference, deterministic runtime behavior, extensibility, domain separation, resource governance, durability, and the relationship between the runtime kernel and specialized agent products.
