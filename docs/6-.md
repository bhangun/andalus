# Andalus Platform — Complete Documentation
...
- [Chapter 1 — What is Andalus?](#chapter-1--what-is-andalus)
  - [1.1 Overview](#11-overview)
  - [1.2 Architectural Boundary](#12-architectural-boundary)
  - [1.3 What Andalus Is Not](#13-what-andalus-is-not)
  - [1.4 The Andalus Equation](#14-the-andalus-equation)
...
- [Chapter 4 — Andalus Architecture Deep Dive](#chapter-4--andalus-architecture-deep-dive)
...
- [Chapter 10 — Extending Andalus](#chapter-10--extending-andalus)
...
1. **Chapters 1–3** — the foundational material (What is Andalus, Design Principles, Runtime Execution Model) drawn from the runtime documentation snapshot (`# Andalus — Agent Runtime & Framework Documentation` and `# 2. Design Principles`).
...
# Chapter 1 — What is Andalus?

## 1.1 Overview

Andalus is an **extensible agent runtime and framework** for building, executing, integrating, and operating AI agents.
...
$$
\boxed{\text{Andalus} = \text{Execution Core} + \text{Abstractions} + \text{Pluggable Implementations}}
$$
...
| Part | Chapters | Focus |
|------|----------|-------|
| I — Foundations | 1–3 | What Andalus is, design principles, execution model |
...
Andalus places important system boundaries behind interfaces and SPIs. Explicit abstractions include:
...
Andalus does not require every capability inside one enormous agent class. Instead, an agent is assembled from cooperating capabilities.
...
A reasoning loop can consume model tokens, tool calls, execution time, memory, retries, and external resources. Andalus therefore models execution budgets explicitly.
...
Andalus treats runtime context as something that can be **planned and compiled**. The runtime has a `RuntimeContextPlanner` and supports planning context against an `ExecutionBudget` and available `ContextProvider` instances.
...
Andalus distinguishes four concepts that are often incorrectly treated as the same thing:
...
Andalus has a durable event abstraction through `EventLedger`. The event ledger is explicitly different from ephemeral callbacks and is intended to support persistent execution history and auditing.
...
Andalus never summarizes context. It reduces *scope* (which members are shown), never *fidelity* (how they're shown). Every surviving character is a substring of the original source.
...
Canonical models live in `andalus-communication-api`. A2A, ANP, and local protocols are *projections* of those canonical models — never the reverse.
...
The core distinction that Andalus makes:
...
| Aspect | Skill | Tool |
|--------|-------|------|
| **Nature** | Internal, versioned Andalus capability | Callable capability exposed to agent |
...
2. User home — `~/.andalus/mcp/mcp.json`
...
This is how Andalus achieves **agent-as-tool** composition.
...
# or
andalus.router.strategy=adaptive
# or
andalus.router.strategy=advanced  # maps to adaptive
```

Or via environment:

```bash
ANDALUS_ROUTER_STRATEGY=adaptive
```
...
## 26.7 Andalus Agent Node Executor
...
Gamelan calls `AndalusAgentNodeExecutor.execute(task)`. The executor resolves bindings, builds `AgentRequest`, invokes runtime.
