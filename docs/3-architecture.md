...
Andalus is organized as a layered execution architecture.
...
## The current implementation has a top-level `DefaultAndalusRuntime`, which delegates execution to `DefaultExecutionEngine`. That engine creates an `AgentExecution` through `AgentExecutionService`, which in turn wires providers, model routing, context planning, memory, caching, events, checkpoints, and tool execution into `DefaultAgentExecution`.

# 3.2 Architectural View

The conceptual Andalus architecture is:
...
Andalus can be described through the following layers.

| Layer                | Responsibility                               |
| -------------------- | -------------------------------------------- |
| Application Layer    | Applications consuming Andalus                |
...
Andalus also exposes an `Agent` SPI.
...
This gives Andalus two complementary concepts:
...
This enables Andalus to represent execution as a first-class runtime object.
...
Andalus separates several intelligence-related responsibilities.
...
The provider abstraction allows the same Andalus execution model to operate across different inference environments.
...
Andalus exposes memory as an abstraction rather than tying it to one storage engine.
...
The architecture can therefore support different knowledge implementations without embedding a particular domain into Andalus.
...
Andalus separates execution state from durable history.
...
Andalus has a workflow abstraction and an integration path for Gamelan.

The architecture allows Gamelan workflow nodes to invoke Andalus agents through a `AndalusAgentNodeExecutor`. The executor resolves workflow inputs, constructs an `AgentRequest`, resolves an agent definition, and invokes `DefaultAndalusRuntime.executeAsync(...)`.
...
This means Andalus can act as an intelligent execution node inside a broader workflow engine.
...
The important architectural point is that remote communication should terminate at the Andalus runtime boundary rather than bypassing the execution kernel.

---

# 3.26 Extension Architecture

Andalus is designed to allow external implementations to plug into the runtime.
...
sequenceDiagram
    ...
    participant RT as AndalusRuntime
...
The source explicitly documents the top-level chain from `DefaultAndalusRuntime` through `DefaultExecutionEngine`, `AgentExecutionService`, `DefaultAgentExecution`, the ReAct loop, and the tool executor.

---

# 3.31 Dependency Direction

A healthy Andalus dependency graph should look approximately like:
...
Andalus can be embedded into an application.
...
flowchart LR
    ...
    APP --> ANDALUS[Embedded Andalus Runtime]

    ANDALUS --> AGENT[Agent]
    ANDALUS --> MODEL[Local / Remote Model]
    ANDALUS --> TOOLS[Tools]
```

This is appropriate when Andalus is used as a library rather than as a standalone service.
...
flowchart LR
    ...
    CLIENT --> API[Andalus API]

    API --> RUNTIME[Andalus Runtime]
...
# 3.36 Architecture Boundary: What Belongs in Andalus Core?
...
$$
\boxed{
\text{Generic mechanism in Andalus}
\quad+\quad
...
Andalus's architecture can ultimately be summarized as:
