...
Andalus is designed as a **general-purpose agent runtime and framework**, rather than as a single agent implementation or a framework tightly coupled to one model provider.
...
Andalus places important system boundaries behind interfaces and SPIs.
...
Andalus does not require every capability to be implemented inside one enormous agent class.
...
This enables specialized projects to build on Andalus without modifying the core runtime.
...
while continuing to use the same Andalus execution foundation.

---

## 2.5 Principle 4 — Provider and Model Independence

Andalus should not make the execution engine fundamentally dependent on one model vendor or inference implementation.
...
Andalus therefore models execution budgets explicitly.
...
Andalus treats runtime context as something that can be **planned and compiled**.
...
Andalus treats execution as more than a single function call.
...
Andalus distinguishes several concepts that are often incorrectly treated as the same thing:
...
This gives Andalus a clean execution model:
...
Andalus has a durable event abstraction through `EventLedger`.
