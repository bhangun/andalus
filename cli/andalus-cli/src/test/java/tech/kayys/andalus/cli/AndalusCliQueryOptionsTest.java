...
class AndalusCliQueryOptionsTest {

    @Test
    void skillOptionsMapToSdkQuery() {
        AndalusSkillQueryOptions options = new AndalusSkillQueryOptions();
    ...
    void contractOptionsMapToSdkQuery() {
        AndalusContractQueryOptions options = new AndalusContractQueryOptions();
        options.schema = " andalus.run.planning ";
        options.envelope = " run-preview ";
        options.commandId = " run-dry-json ";
        options.domain = " planning ";
        options.jsonSchemaId = " urn:andalus:contract:andalus.run.planning:v1:run-preview ";

        AndalusContractQuery query = options.toQuery();

        assertThat(query.schema()).isEqualTo("andalus.run.planning");
        ...
        assertThat(query.jsonSchemaId()).isEqualTo("urn:andalus:contract:andalus.run.planning:v1:run-preview");
        assertThat(query.filtered()).isTrue();
    }

    @Test
    void commandOptionsMapToSdkQuery() {
        AndalusCommandQueryOptions options = new AndalusCommandQueryOptions();
        options.surfaceId = " assistant-agent ";
        options.profileId = " assistant-agent ";
        options.category = " Runs ";
        options.commandId = " run-session-context ";
        options.contractJsonSchemaId = " urn:andalus:contract:andalus.run.lifecycle:v1:run-status ";
        ...
        assertThat(query.contractJsonSchemaId()).isEqualTo("urn:andalus:contract:andalus.run.lifecycle:v1:run-status");
        assertThat(query.filtered()).isTrue();
    }

    @Test
    void runEventOptionsMapToSdkQuery() {
        AndalusRunEventQueryOptions options = new AndalusRunEventQueryOptions();
    ...
    void runHistoryOptionsMapToSdkQuery() {
        AndalusRunHistoryQueryOptions options = new AndalusRunHistoryQueryOptions();
