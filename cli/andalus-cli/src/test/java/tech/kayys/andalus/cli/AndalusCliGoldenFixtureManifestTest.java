...
class AndalusCliGoldenFixtureManifestTest {

    @Test
    void manifestCoversGoldenFixtureCatalogInOrder() {
        List<String> fixtureNames = AndalusCliGoldenFixtures.all().stream()
                .map(AndalusCliGoldenFixtures.GoldenFixture::name)
                .toList();
        List<String> manifestNames = AndalusCliGoldenFixtureManifest.entries().stream()
                .map(AndalusCliGoldenFixtureManifest.Entry::name)
                .toList();
    ...
    void manifestCarriesExplicitSchemaMetadata() {
        AndalusCliGoldenFixtureManifest.Entry entry = entry("status-json.golden");

        assertThat(entry.sdkSource()).isEqualTo("local");
        assertThat(entry.schemaMode()).isEqualTo("explicit");
        assertThat(entry.schema()).isEqualTo("andalus.platform.catalog");
        assertThat(entry.envelope()).isEqualTo("platform-status");
        assertThat(entry.jsonSchemaId())
                .isEqualTo("urn:andalus:contract:andalus.platform.catalog:v1:platform-status");
    ...
    void manifestCarriesLifecycleFixtureMetadata() {
        AndalusCliGoldenFixtureManifest.Entry entry = entry("run-cancel-json.golden");
    ...
    void manifestExposesTypedSchemaModePredicates() {
        AndalusCliGoldenFixtureManifest.Entry explicit = entry("status-json.golden");
        AndalusCliGoldenFixtureManifest.Entry selfDescribing = entry("run-result-json.golden");
        AndalusCliGoldenFixtureManifest.Entry unvalidated = entry("contracts-json.golden");
