...
/**
 * Text renderer for product profile catalog and detail responses shown by the Andalus CLI.
 */
final class AndalusProfileTextFormat {

    private AndalusProfileTextFormat() {
    }

    static String text(String productName, String surfaceId, List<ProductProfile> profiles) {
        StringBuilder output = new StringBuilder("Andalus product profiles\n");
    ...
    static String detailText(String productName, ProductProfile profile) {
        StringBuilder output = new StringBuilder("Andalus product profile\n");
        output.append("product: ").append(productName).append('\n');
        appendProfile(output, profile);
        return output.append('\n').toString();
    }
