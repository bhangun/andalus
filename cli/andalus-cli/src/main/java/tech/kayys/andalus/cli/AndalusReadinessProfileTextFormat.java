 ...
 */
final class AndalusReadinessProfileTextFormat {

    private AndalusReadinessProfileTextFormat() {
    }

    static String text(List<AndalusPlatformReadinessProfileDescriptor> profiles) {
        StringBuilder output = new StringBuilder("Andalus readiness profiles\n");
        List<AndalusPlatformReadinessProfileDescriptor> model = CliLists.copy(profiles);
        output.append("totalProfiles: ").append(model.size()).append('\n');
        for (AndalusPlatformReadinessProfileDescriptor profile : model) {
            appendProfile(output, profile);
        }
        return output.append('\n').toString();
    }

    static String detailText(AndalusPlatformReadinessProfileDescriptor profile) {
        StringBuilder output = new StringBuilder("Andalus readiness profile\n");
        appendProfile(output, profile);
        return output.append('\n').toString();
    }

    static String validationText(AndalusPlatformReadinessProfileValidationReport report) {
        StringBuilder output = new StringBuilder("Andalus readiness profile validation\n");
        ...
        if (!report.issues().isEmpty()) {
            ...
            for (AndalusPlatformReadinessProfileValidationIssue issue : report.issues()) {
                output.append("  - ")
                        .append(issue.kind())
                        .append(": ")
    ...
    static String validationPoliciesText(
        ...
        StringBuilder output = new StringBuilder("Andalus readiness profile validation policies\n");
        List<AndalusPlatformReadinessProfileValidationPolicyDescriptor> model = CliLists.copy(policies);
        output.append("totalPolicies: ").append(model.size()).append('\n');
        for (AndalusPlatformReadinessProfileValidationPolicyDescriptor policy : model) {
            output.append('\n')
    ...
    static String registryResolutionText(AndalusPlatformReadinessProfileRegistryResolution resolution) {
        StringBuilder output = new StringBuilder("Andalus readiness profile sources\n");
        ...
        output.append("profileIds: ")
                .append(CliText.commaSeparated(resolution.profiles().stream()
                        .map(AndalusPlatformReadinessProfileDescriptor::profileId)
                        .toList()))
                .append('\n');
        output.append("validationPolicy: ")
        ...
        for (AndalusPlatformReadinessProfileSourceStatus source : resolution.sources()) {
            output.append("  ")
    ...
    static String registryPreflightText(
        ...
        StringBuilder output = new StringBuilder("Andalus readiness profile registry preflight\n");
    ...
    static String externalReaderProviderDiscoveryText(
        ...
        StringBuilder output = new StringBuilder("Andalus readiness profile external reader providers\n");
        output.append("ready: ").append(CliText.yesNo(report.ready())).append('\n');
        ...
        if (!report.providers().isEmpty()) {
            ...
            for (AndalusPlatformReadinessProfileExternalReaderProviderDiagnostics provider : report.providers()) {
                output.append("  ")
    ...
    static String registryConfigDiagnosticsText(
        ...
        AndalusPlatformReadinessProfileRegistryConfig config = diagnostics.config();
        Map<String, Object> redactedConfig = config.toMap();
        StringBuilder output = new StringBuilder("Andalus readiness profile registry config\n");
        ...
        if (!diagnostics.issues().isEmpty()) {
            output.append('\n').append("Issues").append('\n');
            for (AndalusPlatformReadinessProfileRegistryConfigIssue issue : diagnostics.issues()) {
                output.append("  - ")
                        .append(issue.code())
                        .append(" [")
                        .append(issue.field())
    ...
    private static void appendProfile(StringBuilder output, AndalusPlatformReadinessProfileDescriptor profile) {
        output.append('\n')
