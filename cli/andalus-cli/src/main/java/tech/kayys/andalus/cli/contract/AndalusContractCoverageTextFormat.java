 ...
 */
final class AndalusContractCoverageTextFormat {

    private AndalusContractCoverageTextFormat() {
    }

    static String text(String productName, AndalusContractCommandCoverageReport report) {
        AndalusContractCommandCoverageReport model = AndalusContractEnvelopes.normalizeCoverageReport(report);
    ...
    private static void appendEntries(
            ...
            String title,
            List<AndalusContractCommandCoverageEntry> entries) {
        if (entries.isEmpty()) {
            return;
        }
        output.append(System.lineSeparator()).append(title).append(System.lineSeparator());
        for (AndalusContractCommandCoverageEntry entry : entries) {
            output.append("  - ")
                    .append(entry.schema())
                    .append("/")
