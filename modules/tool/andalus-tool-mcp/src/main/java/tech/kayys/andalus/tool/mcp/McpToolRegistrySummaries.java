...
final class McpToolRegistrySummaries {
    ...
    static McpToolRegistrySummary from(List<tech.kayys.andalus.tool.entity.McpTool> tools) {
    ...
    private static final class RegistrySummaryAccumulator {
        ...
        private void add(tech.kayys.andalus.tool.entity.McpTool tool, String candidateEndpoint) {
            total++;
            lifecycle.add(tool);
            if (tool.isReadOnly()) {
                readOnly++;
