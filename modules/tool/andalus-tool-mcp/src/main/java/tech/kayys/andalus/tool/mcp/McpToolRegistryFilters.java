...
record McpToolRegistryFilters(
    ...
    boolean matches(tech.kayys.andalus.tool.entity.McpTool tool) {
        return tool != null
    ...
    private boolean supportsCapability(tech.kayys.andalus.tool.entity.McpTool tool) {
        if (capability == null) {
            return true;
        }
    ...
    private boolean matchesEnabled(tech.kayys.andalus.tool.entity.McpTool tool) {
        return enabled == null || tool.isEnabled() == enabled;
    }

    private boolean matchesServerName(tech.kayys.andalus.tool.entity.McpTool tool) {
        if (serverName == null) {
            return true;
        }
    ...
    private boolean matchesStale(tech.kayys.andalus.tool.entity.McpTool tool) {
        return stale == null || McpToolLifecycle.isStale(tool) == stale;
    }

    private boolean matchesServerDisabled(tech.kayys.andalus.tool.entity.McpTool tool) {
        return serverDisabled == null || McpToolLifecycle.isServerDisabled(tool) == serverDisabled;
    }

    private boolean matchesRetired(tech.kayys.andalus.tool.entity.McpTool tool) {
        return retired == null || McpToolLifecycle.isRetired(tool) == retired;
    }

    private boolean matchesLifecycleState(tech.kayys.andalus.tool.entity.McpTool tool) {
