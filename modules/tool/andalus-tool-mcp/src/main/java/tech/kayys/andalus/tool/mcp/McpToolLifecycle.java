...
final class McpToolLifecycle {
    ...
    static boolean belongsToServer(tech.kayys.andalus.tool.entity.McpTool tool, String serverName) {
    ...
    static String serverName(tech.kayys.andalus.tool.entity.McpTool tool) {
        if (tool == null) {
            return null;
        }
    ...
    static boolean isStale(tech.kayys.andalus.tool.entity.McpTool tool) {
        return tool != null
                && (hasValueIgnoreCase(tool.getTags(), STALE_TAG)
                || hasValueIgnoreCase(tool.getCapabilities(), STALE_TAG));
    }

    static boolean isServerDisabled(tech.kayys.andalus.tool.entity.McpTool tool) {
        return tool != null && hasValueIgnoreCase(tool.getTags(), SERVER_DISABLED_TAG);
    }

    static boolean isRetired(tech.kayys.andalus.tool.entity.McpTool tool) {
        return tool != null && hasValueIgnoreCase(tool.getTags(), SERVER_RETIRED_TAG);
    }

    static String lifecycleState(tech.kayys.andalus.tool.entity.McpTool tool) {
        if (isRetired(tool)) {
