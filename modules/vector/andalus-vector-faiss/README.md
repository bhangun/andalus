# Andalus FAISS Vector Store

Native FAISS (Facebook AI Similarity Search) integration for the Andalus platform via **JDK 25 FFM** (Foreign Function & Memory API).
...
This clones FAISS to `~/.andalus/vendor/faiss`, builds `libfaiss_c`, and copies it to `~/.andalus/lib/`.
...
andalus.vector.store.type=faiss

# Vector dimension (default: 768)
andalus.vector.faiss.dimension=768

# FAISS index factory string (default: Flat)
# Examples: "Flat", "HNSW32", "IVF100,Flat", "PQ16", "SQ8", "IVF256,PQ32"
andalus.vector.faiss.index.type=Flat

# Index persistence path (optional)
andalus.vector.faiss.index.path=/path/to/index.faiss
```
...
<dependency>
    ...
    <artifactId>andalus-vector-faiss</artifactId>
