package tech.kayys.andalus.vector.pgvector;
...
class PgVectorStoreTest {
    ...
    void setUp() {
        ...
        pgPool.query("TRUNCATE TABLE andalus_vector_entries").executeAndForget();
    }
    ...
    void testInitialize() {
        ...
        Boolean tableExists = pgPool.preparedQuery(
                "SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'andalus_vector_entries')")
                .execute()
    ...
    void testStoreAndRetrieve() {
        ...
        Long count = pgPool.preparedQuery("SELECT COUNT(*) FROM andalus_vector_entries WHERE id = $1")
    ...
    void testDeleteById() {
        ...
        // Verify it exists
        Long countBefore = pgPool.preparedQuery("SELECT COUNT(*) FROM andalus_vector_entries WHERE id = $1")
        ...
        // Verify it's gone
        Long countAfter = pgPool.preparedQuery("SELECT COUNT(*) FROM andalus_vector_entries WHERE id = $1")
                .execute(Tuple.of("delete-test-1"))
                .map(rowSet -> rowSet.iterator().next().getLong(0))
                .await().indefinitely();
    ...
    void testDeleteByFilters() {
        ...
        Long countBefore = pgPool.preparedQuery("SELECT COUNT(*) FROM andalus_vector_entries WHERE metadata->>'category' = $1")
        ...
        Long countAfter = pgPool.preparedQuery("SELECT COUNT(*) FROM andalus_vector_entries WHERE metadata->>'category' = $1")
                .execute(Tuple.of("tech"))
                .map(rowSet -> rowSet.iterator().next().getLong(0))
                .await().indefinitely();
