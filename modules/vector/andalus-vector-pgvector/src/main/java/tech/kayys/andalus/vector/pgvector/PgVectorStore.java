...
public class PgVectorStore extends AbstractVectorStore {
    ...
    @ConfigProperty(name = "andalus.vector.pgvector.dimension", defaultValue = "1536")
    int vectorDimension;

    @ConfigProperty(name = "andalus.vector.pgvector.index.type", defaultValue = "hnsw")
    ...
    public Uni<Void> initialize() {
        ...
        String createTableSql = """
                CREATE TABLE IF NOT EXISTS andalus_vector_entries (
                ...
                CREATE INDEX idx_vector_entries_embedding_%s
                    ON andalus_vector_entries USING %s (embedding vector_cosine_ops);

                -- Index for faster metadata lookups
                CREATE INDEX IF NOT EXISTS idx_vector_entries_metadata_gin
                    ON andalus_vector_entries USING GIN (metadata);
    ...
    private Uni<Void> storeSingle(VectorEntry entry) {
        String sql = """
                INSERT INTO andalus_vector_entries (id, content, embedding, metadata)
    ...
    public Uni<Void> delete(List<String> ids) {
        ...
        String sql = "DELETE FROM andalus_vector_entries WHERE id = ANY($1)";
    ...
    public Uni<Void> deleteByFilters(Map<String, Object> filters) {
        ...
        StringBuilder sql = new StringBuilder("DELETE FROM andalus_vector_entries WHERE ");
        List<Object> params = new ArrayList<>();
        boolean first = true;
