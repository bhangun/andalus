#!/usr/bin/env bash
#
# Andalus Platform Installation Script
...
REPO_NAME="andalus" # Replace with actual repo if different
GITHUB_API="https://api.github.com/repos/$REPO_OWNER/$REPO_NAME"

INSTALL_DIR="${HOME}/.local/bin"
ANDALUS_HOME="${HOME}/.andalus"
ANDALUS_BIN_DIR="${ANDALUS_HOME}/bin"
...
install_native() {
  local asset_name="andalus-standalone-${OS_NAME}-${ARCH_NAME}"
  ...
  mkdir -p "$INSTALL_DIR"
  local target_file="${INSTALL_DIR}/andalus"
  if [ "$OS_NAME" = "windows" ]; then
    target_file="${INSTALL_DIR}/andalus.exe"
  fi

  curl -sSL -o "$target_file" "$DOWNLOAD_URL"
  chmod +x "$target_file"
  
  log "Successfully installed native Andalus to $target_file"
}
...
install_jvm() {
  ...
  # Release action creates: artifacts/andalus-runtime-standalone-<version>.jar
  local release_version="${RELEASE_TAG#v}"
  local asset_name="andalus-runtime-standalone-${release_version}.jar"
  
  log "Looking for JVM asset: $asset_name"
  DOWNLOAD_URL=$(echo "$RELEASE_JSON" | jq -r ".assets[] | select(.name | test(\"andalus-runtime-standalone-.*\\\\.jar\")) | .browser_download_url" | head -n 1)
  ...
  mkdir -p "$ANDALUS_BIN_DIR"
  local target_jar="${ANDALUS_BIN_DIR}/andalus-runtime-standalone.jar"
  
  curl -sSL -o "$target_jar" "$DOWNLOAD_URL"
  
  # Create wrapper
  mkdir -p "$INSTALL_DIR"
  local wrapper_file="${INSTALL_DIR}/andalus"
  
  cat << 'EOF' > "$wrapper_file"
#!/usr/bin/env bash
exec java -jar "${HOME}/.andalus/bin/andalus-runtime-standalone.jar" "$@"
EOF
  chmod +x "$wrapper_file"

  log "Successfully installed JVM Andalus wrapper to $wrapper_file"
}
...
check_path
log "Installation complete! Run 'andalus /help' to get started."
