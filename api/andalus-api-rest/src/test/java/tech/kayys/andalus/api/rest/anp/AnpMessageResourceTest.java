...
import tech.kayys.andalus.anp.auth.AnpHttpSignature;
...
class AnpMessageResourceTest {
    ...
    void acceptsMessageAndSignsResponse() {
        AnpMessageResource.AnpMessageRequest request = new AnpMessageResource.AnpMessageRequest(
                ...
                "did:wba:test.andalus.local:andalus",
                "a2a/1.0",
                "Hello Andalus Agent",
                "tenant-1",
                Map.of()
        ...
        AnpHttpSignature sig = authenticator.sign("POST", "/api/v1/anp/message", "test.andalus.local", clientKey);
        String sigHeader = "Signature " + sig.toHeaderValue();

        Response response = resource.handleMessage(sigHeader, null, "test.andalus.local", request);
        ...
        assertTrue(entity.payload().contains("Hello Andalus Agent"));
    }
